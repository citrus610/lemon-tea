...
*/
bool Tree::set(BitBoard board, PieceType current, PieceType hold, PieceType next[MAX_TREE_QUEUE], int next_count, int b2b, int ren)
{
	// Reset root node
	this->root = Node();
	this->root.state = {
...
int Tree::search(int iteration)
{
	...
	while (iter_num < iteration) {
		search_one_iter(iter_num, layer_index, width, node_count);
		++iter_num;
	}
...
*/
void Tree::search_one_iter(int& iter_num, int& layer_index, int& width, int& node_count)
{
	...
	// Check backtracking, forecast and find the layer to expand
	bool backtrack = false;
	while (this->layer[layer_index].get_size() == 0) {
		backtrack = true;
		width = this->beam_backtrack;
		if (layer_index > 0)
			--layer_index;
		else
			return;
	...
	// Expand the layer
	expand_layer(this->layer[layer_index], this->layer[layer_index + 1], width, node_count);

	// If backtrack, then prune nodes
	if (backtrack) {
		if (this->layer[layer_index].get_size() < width * 32) {
