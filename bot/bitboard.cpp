...
bool BitBoard::is_occupied(const int8_t& x, const int8_t& y)
{
	if (x < 0 || x > 9 || y < 0 || y > 39) return true;
	return (column[x] & ((uint64_t)0b1 << y)) != 0;
}

bool BitBoard::is_colliding(const int8_t& x, const int8_t& y, const PieceType& type, const PieceRotation& rotation)
{
	for (int i = 0; i < 4; ++i) {
...
bool BitBoard::piece_try_rotate(PieceData& piece, bool is_cw)
{
	...
	PieceRotation new_rotation = (PieceRotation)(((int8_t)piece.rotation + (!is_cw * 2 + 1)) % 4);
	int8_t srs_piece_index = std::min((int8_t)piece.type, (int8_t)1);
	for (int i = 0; i < 5; ++i) {
		int8_t offset_x = SRS_LUT[srs_piece_index][piece.rotation][i][0] - SRS_LUT[srs_piece_index][new_rotation][i][0];
		int8_t offset_y = SRS_LUT[srs_piece_index][piece.rotation][i][1] - SRS_LUT[srs_piece_index][new_rotation][i][1];
