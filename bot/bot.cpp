...
// Start a bot thread
void Bot::init_thread(BotSetting setting, PieceType queue[MAX_TREE_QUEUE], int queue_count)
{
	...
	// Init bot state
	BotState state;
	memcpy(state.queue, queue, std::min(queue_count, MAX_TREE_QUEUE) * sizeof(PieceType));
	state.queue_count = std::min(queue_count, MAX_TREE_QUEUE);
	...
	this->thread = new std::thread([&](BotSetting bot_setting, BotState bot_state)
		{
			// Init tree
			Tree tree;
			tree.init(bot_state.queue, bot_state.queue_count);
			...
			{
				// Search one iter
				tree.think(iter_num, layer_index, node_count);
				++iter_num;
				...
				if (!state_buffer.empty()) {
					...
					tree.reset(state_buffer[0].board, state_buffer[0].b2b, state_buffer[0].ren);
				...
				if (!action_buffer.empty()) {
					...
					bool success = tree.advance(action_buffer[0].action, action_buffer[0].new_piece, action_buffer[0].new_piece_count);
