...
void beam::forecast()
{
	...
	while (true)
	{
		...
		// This is not really random, since the first two piece of the bag will alway be I and T to boost the bot's confidence
		for (int n = 2; n < 7; ++n) {
			int to_swap_index = rand() % 5 + 2;
			piece_type place_holder = bag[n];
			bag[n] = bag[to_swap_index];
...
{
	reset(); // Not sure if this should be here
	root = node();
	root.board = board;
...
void beam::expand_node(node& parent, vec<node>& new_layer, const bool& is_first, const bool& is_last, int& node_count)
{
	...
	if (is_last) {
		...
		for (int i = 0; i < current_piece_pos_count; ++i) {
			...
			evaluator.evaluate(new_layer.pointer[new_layer.size - 1]);
			if (is_first) { 
