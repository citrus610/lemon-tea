...
void Spsa::vary_weight(LemonTea::Weight& base, LemonTea::Weight& v1, LemonTea::Weight& v2)
{
    ...
    VARY_WEIGHT_PARAMETER(defence.bumpiness, 50);
    VARY_WEIGHT_PARAMETER(defence.bumpiness_s, 10);
    VARY_WEIGHT_PARAMETER(defence.flat, 50);
    VARY_WEIGHT_PARAMETER(defence.row_t, 50);
    VARY_WEIGHT_PARAMETER(defence.column_t, 50);
    VARY_WEIGHT_PARAMETER(defence.hole, 100);
    VARY_WEIGHT_PARAMETER(defence.crack, 100);
    VARY_WEIGHT_PARAMETER(defence.blocked, 25);
    VARY_WEIGHT_PARAMETER(defence.well, 20);
    for (int i = 0; i < 4; ++i) {
    ...
    // Attack
    for (int i = 0; i < 4; ++i) {
        VARY_WEIGHT_PARAMETER(attack.clear[i], 100);
    }
    for (int i = 0; i < 3; ++i) {
        VARY_WEIGHT_PARAMETER(attack.tspin[i], 100);
    }
    VARY_WEIGHT_PARAMETER(attack.waste_time, 25);
    VARY_WEIGHT_PARAMETER(attack.waste_T, 100);
    VARY_WEIGHT_PARAMETER(attack.b2b, 100);
    VARY_WEIGHT_PARAMETER(attack.ren, 100);
}
...
void Spsa::approach_weight(LemonTea::Weight& base, LemonTea::Weight& v)
{
    ...
    APPROACH_WEIGHT_PARAMETER(defence.column_t, ap_v);
