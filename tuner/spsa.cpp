...
void Spsa::vary_weight(LemonTea::Weight& base, LemonTea::Weight& v1, LemonTea::Weight& v2)
{
    ...
    // VARY_WEIGHT_PARAMETER(defence.row_t, 25);
    // VARY_WEIGHT_PARAMETER(defence.hole_a, 100);
    // VARY_WEIGHT_PARAMETER(defence.hole_b, 100);
    // VARY_WEIGHT_PARAMETER(defence.blocked, 35);
    VARY_WEIGHT_STATIC_PARAMETER(defence.well, ((rand() & 2) * 2 - 1) * (10 + (rand() % 10)));
    VARY_WEIGHT_STATIC_PARAMETER(defence.sidewell, ((rand() & 2) * 2 - 1) * (10 + (rand() % 10)));
    // for (int i = 0; i < 4; ++i) {
    ...
    VARY_WEIGHT_PARAMETER(attack.ren, 50);
    VARY_WEIGHT_PARAMETER(attack.ren_10, 25);
    VARY_WEIGHT_PARAMETER(attack.ren_15, 25);
};
...
void Spsa::approach_weight(LemonTea::Weight& base, LemonTea::Weight& v)
{
    ...
    APPROACH_WEIGHT_PARAMETER(defence.sidewell, ap_v);
    for (int i = 0; i < 4; ++i) {
    ...
    APPROACH_WEIGHT_PARAMETER(attack.ren_10, ap_v);
    APPROACH_WEIGHT_PARAMETER(attack.ren_15, ap_v);
};
